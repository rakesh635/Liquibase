pipeline {
    ...
    stages {
        ...
        stage('Build') {
            steps {
                ...
                echo 'workspace...' + env.WORKSPACE
                    ...
                    //sh "mvn sonar:sonar clean compile -Dtest=\\!TestRunner* -DfailIfNoTests=false -Dsonar.projectKey=employee_jdbc -Dsonar.host.url=http://34.93.123.206/ -Dsonar.login=7c050a14938e26d649426d0d10b5d95d1fb3be39"
            }
        }
        stage('Package') {
            steps {
        ...
        stage('Deploy') {
            steps {
                sh 'curl --upload-file target/SpringMVCHibernateCRUD.war "http://tomcat:password@10.62.125.4:8083/manager/text/deploy?path=/curd1&update=true"'
                //sh 'curl --upload-file target/SpringMVCHibernateCRUD.war "http://tomcat:password@34.93.238.186:8081/manager/text/deploy?path=/curd&update=true"'
